...
const memoryProxy = new Proxy(importObject.intrinsics.memory, {
    get(target, prop, receiver) {
        if (memoryFirstTimeAccess) {
            ...
            console.error('memory is deprecated. Consider to use kotlin.wasm.unsafe.wasmMemory or try to update your dependencies');
        }
...
const wasmExports = new Proxy(memoryProxy, {
    get(target, prop, receiver) {
        if (prop == 'memory') {
            ...
            throw new Error('wasmExports is not available anymore. Please remove this usage from the code or try to update project dependencies.');
        }
    }
});
""" else ""
    ...
    val memoryImport = stdlibModule?.fileName?.let { "import { wasmExports } from './$it.mjs'" } ?: ""
